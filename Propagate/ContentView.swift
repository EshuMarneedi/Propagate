...
struct ContentView: View {
    ...
    var body: some View {
        ...
        // When a user opens a video from Files, the video is added to the model, and selectedVideoURL is set to that new openedURL. (We get openedURL from videoURL in the Scene.)
        .onChange(of: openedURL) {
            if let openedURL {
                addVideo(from: openedURL)
                selectedVideoURL = IdentifiableURL(url: openedURL)
                self.openedURL = nil
            }
        }
    }
